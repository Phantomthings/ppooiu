...
var (
	...
	// Champs BATT
	fieldSEQ02PC1 = "SEQ02.OLI.A.PC1" // DC1
	fieldSEQ02IC1 = "SEQ02.OLI.A.IC1"
	fieldSEQ03PC1 = "SEQ03.OLI.A.PC1" // DC2
	fieldSEQ03IC1 = "SEQ03.OLI.A.IC1"

	// Regles
	minIndispoMinutes = envInt("MIN_INDISPO_MINUTES", 5)
	safetyLagMinutes  = envInt("SAFETY_LAG_MINUTES", 5)
...
// Influx fetch
func fetchMinuteGridUTC(cli influx.Client, project string, startUTC, endUTC time.Time) (map[time.Time]pair, map[time.Time]pair, error) {
...
`,
		fieldSEQ02PC1, fieldSEQ02PC1,
		fieldSEQ02IC1, fieldSEQ02IC1,
		fieldSEQ03PC1, fieldSEQ03PC1,
		fieldSEQ03IC1, fieldSEQ03IC1,
		influxMeas, startISO, endISO, influxTagProject, project)

	resp, err := cli.Query(influx.NewQuery(q, influxDB, ""))
	if err != nil {
		return nil, nil, fmt.Errorf("influx query: %w", err)
	}
	if resp.Error() != nil {
		return nil, nil, fmt.Errorf("influx resp: %w", resp.Error())
	}

	dc1 := map[time.Time]pair{}
	dc2 := map[time.Time]pair{}
	...
	if len(resp.Results) > 0 && len(resp.Results[0].Series) > 0 {
		...
		if len(cols) >= 5 {
			...
			for _, v := range s.Values {
				...
				p1 := toIntPtr(v[iDC1PC])
				i1 := toIntPtr(v[iDC1IC])
				p2 := toIntPtr(v[iDC2PC])
				i2 := toIntPtr(v[iDC2IC])

				dc1[t.UTC()] = pair{pc1: p1, ic1: i1}
				dc2[t.UTC()] = pair{pc1: p2, ic1: i2}
			}
		}
	}
	...
	for t := startUTC.UTC(); t.Before(endUTC.UTC()); t = t.Add(time.Minute) {
		if _, ok := dc1[t]; !ok {
			dc1[t] = pair{pc1: nil, ic1: nil}
		}
		if _, ok := dc2[t]; !ok {
			dc2[t] = pair{pc1: nil, ic1: nil}
		}
	}
	return dc1, dc2, nil
}
...
// Classification
func classify(pc1, ic1 *int) (int, *string) {
	if pc1 == nil || ic1 == nil {
		msg := "Donnée manquante"
		return -1, &msg
	}
	if (*pc1 == 0 && *ic1 == 0) || (*pc1 == 4 && *ic1 == 8) {
		return 1, nil
	}
...
func buildBlocks(site, equip string, series map[time.Time]pair, batchID string) []bloc {
	type row struct {
		t     time.Time
		state int
		cause *string
	...
	for i := 1; i < len(rows); i++ {
		...
		if r.state != curState || c1 != c2 {
			...
			if curState == 0 {
				if int(end.Sub(start).Minutes()) < minIndispoMinutes {
					curState = r.state
					curCause = r.cause
					start = r.t
					rawCount = 0
			...
			h := hashSig(site, equip, start, end, curState, curCause)
...
// SQL
func tableDC1(site string) string {
	return fmt.Sprintf("indicator.dispo_blocs_batt_%s", sanitizeSite(site))
}
func tableDC2(site string) string {
	return fmt.Sprintf("indicator.dispo_blocs_batt2_%s", sanitizeSite(site))
}
...
func upsertStmtDC1(t string) string {
	return fmt.Sprintf(`
INSERT INTO %s
...
func upsertStmtDC2(t string) string {
	return fmt.Sprintf(`
INSERT INTO %s
(site, equipement_id, type_equipement, date_debut, date_fin, est_disponible, cause,
 raw_point_count, processed_at, batch_id, hash_signature)
VALUES (?, 'DC2', 'BATT', ?, ?, ?, ?, ?, UTC_TIMESTAMP(), ?, ?)
ON DUPLICATE KEY UPDATE
  est_disponible=VALUES(est_disponible),
  cause=VALUES(cause),
  raw_point_count=VALUES(raw_point_count),
...
func saveBlocksPerSite(db *sql.DB, site string, blocks []bloc) (int, error) {
	if len(blocks) == 0 {
		return 0, nil
	}
	stmt1, err := db.Prepare(upsertStmtDC1(tableDC1(site)))
	if err != nil {
		return 0, err
	}
	defer stmt1.Close()
	stmt2, err := db.Prepare(upsertStmtDC2(tableDC2(site)))
	if err != nil {
		return 0, err
	}
	defer stmt2.Close()
	...
	for _, b := range blocks {
		...
		var execErr error
		if b.equip == "DC1" {
			_, execErr = tx.Stmt(stmt1).Exec(args...)
		} else {
			_, execErr = tx.Stmt(stmt2).Exec(args...)
		}
		if execErr != nil {
			_ = tx.Rollback()
			return n, execErr
...
func processDayParis(site string, dayParis time.Time, ic influx.Client, db *sql.DB) (int, int, error) {
	startLocal := time.Date(dayParis.Year(), dayParis.Month(), dayParis.Day(), 0, 0, 0, 0, paris)
	endLocal := startLocal.Add(24 * time.Hour)
	return processWindow(site, startLocal.UTC(), endLocal.UTC(), ic, db)
}

func processWindow(project string, startUTC, endUTC time.Time, ic influx.Client, db *sql.DB) (int, int, error) {
	log.Printf("BATT %s : %s → %s (UTC)", project, startUTC.Format(time.RFC3339), endUTC.Format(time.RFC3339))

	dc1m, dc2m, err := fetchMinuteGridUTC(ic, project, startUTC, endUTC)
	if err != nil {
		return 0, 0, err
	}
	...
	blocks := []bloc{}
	blocks = append(blocks, buildBlocks(project, "DC1", dc1m, batchID)...)
	blocks = append(blocks, buildBlocks(project, "DC2", dc2m, batchID)...)

	n, err := saveBlocksPerSite(db, project, blocks)
	if err != nil {
		return 0, 0, err
	}
	n1, n2 := 0, 0
	...
	log.Printf("%s: %d bloc(s) DC1, %d bloc(s) DC2 upsertés", project, n1, n2)
	log.Printf("upsert: %d blocs écrits", n)
	return n1, n2, nil
}
func processDayAuto(site string, day time.Time, ic influx.Client, db *sql.DB) (int, int, error) {
	start := time.Date(day.Year(), day.Month(), day.Day(), 0, 0, 0, 0, time.UTC)
	end := start.Add(24 * time.Hour)
	return processWindow(site, start, end, ic, db)
}
func firstDateInflux(site string, ic influx.Client) (*time.Time, error) {
	q := fmt.Sprintf(`SELECT first("%s") FROM "%s" WHERE "%s"='%s'`,
		fieldSEQ02PC1, influxMeas, influxTagProject, site)
	...
	d := midnightParis(tUTC)
	return &d, nil
}

func lastDateDB(site string, db *sql.DB) (*time.Time, error) {
	var d1, d2 sql.NullTime
	q1 := fmt.Sprintf(`SELECT MAX(date_debut) FROM %s`, tableDC1(site))
	q2 := fmt.Sprintf(`SELECT MAX(date_debut) FROM %s`, tableDC2(site))
	if err := db.QueryRow(q1).Scan(&d1); err != nil {
		return nil, err
	}
	if err := db.QueryRow(q2).Scan(&d2); err != nil {
		return nil, err
	}
	...
	if d2.Valid {
		if best == nil || d2.Time.After(*best) {
			t := d2.Time
			best = &t
		}
	}
...
func autoUpdateAll(projects []string) error {
	...
	for i, site := range projects {
		ld, err := lastDateDB(site, db)
		var startDayP time.Time
		if err != nil {
			log.Printf("lastDateDB %s: %v", site, err)
			continue
		}
		if ld != nil {
			startDayP = midnightParis(*ld).Add(24 * time.Hour)
		} else {
			fd, err := firstDateInflux(site, ic)
			if err != nil {
				log.Printf("firstDateInflux %s: %v", site, err)
				continue
			}
			if fd == nil {
				startDayP = todayP
		...
		for d := startDayP; !d.After(yesterdayP); d = d.Add(24 * time.Hour) {
			n1, n2, err := processDayParis(site, d, ic, db)
			if err != nil {
				log.Printf("processDay %s %s: %v", site, d.In(paris).Format("2006-01-02"), err)
				continue
			}
			total1 += n1
			total2 += n2
	...
	log.Printf("Terminé: %d blocs DC1, %d blocs DC2", total1, total2)
	return nil
}
...
func main() {
	...
	start := flag.String("start-date", "", "YYYY-MM-DD (UTC) — active le mode manuel")
	end := flag.String("end-date", "", "YYYY-MM-DD (exclu, UTC) — active le mode manuel")
	flag.Parse()
	...
	if *start != "" && *end != "" {
		...
		for _, site := range projects {
			n1, n2, err := processWindow(site, startP.UTC(), endP.UTC(), ic, db)
			if err != nil {
		...
		log.Printf("MANUEL terminé: %d blocs DC1, %d blocs DC2", t1, t2)
		return
	}
